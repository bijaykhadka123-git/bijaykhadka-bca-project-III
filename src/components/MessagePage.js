...
const MessagePage = () => {
  ...
  const [tamperedWarningShown, setTamperedWarningShown] = useState(false);
  ...
  useEffect(()=>{
      if(socketConnection){
        ...
        socketConnection.on('message',(data)=>{
          // Server handles decryption, just use the data as is
          setAllMessage(data)
        })
  ...
  const handleSendMessage = (e)=>{
    ...
    if(message.text || message.imageUrl || message.videoUrl){
      if(socketConnection){
        // Server handles encryption, just send plain text
        socketConnection.emit('new message',{
          sender : user?._id,
          receiver : params.userId,
          text : message.text, // Send plain text, server will encrypt
                  ...
                  <div className='flex flex-col gap-2 py-2 mx-2' ref={currentMessage}>
                    {
                      (() => {
                        ...
                        return allMessage.map((msg, index) => {
                          let decryptedText;
                          try {
                          ...
                          if (decryptedText === null) {
                            if (!warningShown) {
                              warningShown = true;
                              return (
                                <div key={msg._id || index} className="bg-yellow-100 text-yellow-800 p-2 rounded">
                                  A message was tampered with and has been hidden for your security!
                                </div>
                              );
                            } else {
                              // Skip rendering this tampered message
                              return null;
                            }
                          }
                          // Determine if the message is sent by the current user
                          const isOwnMessage = user._id === msg?.msgByUserId || user._id === msg?.sender;
                          return (
                            <div key={msg._id || index} className={`p-1 py-1 rounded w-fit max-w-[280px] md:max-w-sm lg:max-w-md ${isOwnMessage ? "ml-auto bg-teal-100" : "bg-white"}`}>
                              ...
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex-1">
                                  <p className='px-2'>{decryptedText}</p>
                                  {msg.edited && (
